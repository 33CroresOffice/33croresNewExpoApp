...
import { ArrowLeft, ShieldCheck, ArrowRight, Clock, Bike } from 'lucide-react-native';
...
export default function RiderOtpVerifyScreen() {
  ...
  const handleVerify = useCallback(async () => {
    ...
    try {
      ...
      // 2. Check rider approval status before signing in
      ...
      if (!rider) {
        setError('No rider profile found for this number. Please register first.');
        setLoading(false);
        return;
      }
      ...
      if (!authData.access_token) {
        setError(authData.error_description || 'Authentication failed. Please try again.');
        setLoading(false);
        return;
      }
      ...
      setSession(authData as any);
      await loadProfile(authData.user.id);

      // Authorize via the riders table, not profile.role — a user can be both customer and rider
      const { data: riderRec } = await supabase
        .from('riders')
        .select('id, approval_status')
        .eq('mobile', mobile)
        .maybeSingle();

      if (!riderRec || riderRec.approval_status !== 'approved') {
        setError('This portal is for approved riders only.');
        setLoading(false);
        return;
      }
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      >
        ...
        <View style={[styles.heroBrand, { top: insets.top + Spacing[3] + 48 + Spacing[3] }]}>
          ...
          <Text style={styles.heroBrandText}>Rider Portal</Text>
        </View>
        ...
        <View style={styles.card}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}
          >
            <Text style={styles.verifyBtnText}>{loading ? 'Verifying...' : 'Verify & Sign In'}</Text>
...
const styles = StyleSheet.create({
  ...
  timerHighlight: { fontFamily: Typography.fontFamily.sansSemiBold, color: Colors.primary },
  expiredText: { fontFamily: Typography.fontFamily.sansMedium, fontSize: Typography.size.sm, color: Colors.error },
  error: {
  ...
  verifyBtnDisabled: { opacity: 0.42 },
  verifyBtnText: {
    fontFamily: Typography.fontFamily.sansSemiBold, fontSize: Typography.size.md,
    color: Colors.white, letterSpacing: 0.2,
  },
  footer: { gap: Spacing[2], alignItems: 'center', marginTop: -Spacing[2] },
  resendRow: { flexDirection: 'row', alignItems: 'center' },
  resendText: { fontFamily: Typography.fontFamily.sansRegular, fontSize: Typography.size.sm, color: Colors.textTertiary },
  resendLink: { fontFamily: Typography.fontFamily.sansSemiBold, fontSize: Typography.size.sm, color: Colors.primary },
  resendDisabled: { color: Colors.textDisabled },
  changeRow: { paddingVertical: Spacing[1] },
  changeText: {
    fontFamily: Typography.fontFamily.sansRegular, fontSize: Typography.size.sm,
