...
import { ArrowLeft, User, Phone, Shield, IndianRupee, CircleAlert as AlertCircle, Check, Pencil, Bike } from 'lucide-react-native';
...
export default function EditRiderScreen() {
  return (
    <ModuleGuard module="riders">
      <EditRiderScreenContent />
    </ModuleGuard>
  );
}

function EditRiderScreenContent() {
  ...
  useEffect(() => {
    if (!id) { setFetchError('No rider ID provided.'); setLoading(false); return; }
    (async () => {
      const { data, error } = await supabase.from('riders').select('*').eq('id', id).single();
      if (error || !data) {
        setFetchError(error?.message ?? 'Rider not found.');
        setLoading(false);
        return;
      }
      setRiderId(data.id);
      setForm({
        ...
        email: data.email ?? '',
        vehicle_type: (data.vehicle_type as VehicleType) ?? 'bike',
        vehicle_number: data.vehicle_number ?? '',
        license_number: data.license_number ?? '',
        zone: data.zone ?? 'General',
        joining_date: data.joining_date ? parseISO(data.joining_date) : new Date(),
        ...
        aadhaar_photo_url: data.aadhaar_photo_url ?? null,
        id_card_type: (data.id_card_type as IdCardType) ?? '',
        id_card_number: data.id_card_number ?? '',
        id_card_photo_url: data.id_card_photo_url ?? null,
        license_photo_url: data.license_photo_url ?? null,
      });
      setLoading(false);
    })();
  }, [id]);

  const p = (key: keyof FormState) => (v: any) =>
  ...
  const save = async () => {
    ...
    const payload: any = {
      ...
      license_photo_url: form.license_photo_url,
      updated_at: new Date().toISOString(),
    };

    const { error } = await supabase.from('riders').update(payload).eq('id', riderId);
    setSaving(false);
  ...
  if (loading) {
    return (
      <View style={[s.root, { paddingTop: WEB ? 0 : insets.top, alignItems: 'center', justifyContent: 'center' }]}>
        <ActivityIndicator size="large" color={Colors.primary} />
        <Text style={s.loadingText}>Loading rider details…</Text>
      </View>
    );
  }
  ...
  if (fetchError) {
    return (
      <View style={[s.root, { paddingTop: WEB ? 0 : insets.top, alignItems: 'center', justifyContent: 'center', padding: Spacing[6] }]}>
    ...
    <View style={[s.root, { paddingTop: WEB ? 0 : insets.top }]}>
      ...
      <View style={s.topBar}>
        <View style={s.topBarInner}>
          ...
          <Text style={s.topBarTitle}>Edit Rider</Text>
          <View style={{ width: 36 }} />
        </View>
      </View>
      ...
      <ScrollView style={s.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <View style={s.heroInner}>
            <View style={s.heroLeft}>
              <View style={s.heroIconCircle}>
                <Pencil size={22} color={Colors.white} strokeWidth={1.8} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={s.heroTitle} numberOfLines={1}>{form.full_name || 'Edit Rider'}</Text>
                <Text style={s.heroSub}>Update rider profile and documents</Text>
              </View>
            </View>
        ...
        <View style={s.contentWrap}>
          ...
          {/* Profile Photo */}
          <Card icon={<User size={15} color={Colors.primary} strokeWidth={2} />} title="Profile Photo" iconBg={Colors.primarySurface}>
            <PhotoUploadField
              label="Rider Photo"
              value={form.profile_photo_url}
              onChange={p('profile_photo_url')}
              storagePath={`riders/${riderId}/profile`}
              aspectRatio={[1, 1]}
          ...
          {/* Personal Details */}
          <Card icon={<User size={15} color="#1565C0" strokeWidth={2} />} title="Personal Details" iconBg="#E3F2FD">
          ...
          {/* Vehicle & Zone */}
          <Card icon={<Bike size={15} color="#6A1B9A" strokeWidth={2} />} title="Vehicle & Zone" iconBg="#F3E5F5">
            <FieldLabel label="Vehicle Type" required />
            <View style={s.chips}>
              {VEHICLE_OPTIONS.map(opt => (
                <Chip key={opt.value} label={opt.label} active={form.vehicle_type === opt.value} onPress={() => setForm(f => ({ ...f, vehicle_type: opt.value }))} />
              ))}
            </View>
            <FieldLabel label="Delivery Zone" required />
            <View style={s.chips}>
              {ZONE_OPTIONS.map(z => (
                <Chip key={z} label={z} active={form.zone === z} onPress={() => setForm(f => ({ ...f, zone: z }))} />
              ))}
            </View>
            <Row>
          ...
          {/* ID Documents */}
          <Card icon={<Shield size={15} color="#00695C" strokeWidth={2} />} title="ID Documents" iconBg="#E0F2F1">
            ...
            <View style={s.aadhaarBox}>
              ...
              <PhotoUploadField
                ...
                storagePath={`riders/${riderId}/aadhaar`}
                aspectRatio={[8, 5]}
            ...
            <View style={s.requiredBox}>
              ...
              <View style={s.chips}>
                {ID_CARD_OPTIONS.map(opt => (
                  <Chip
                    ...
                    onPress={() => setForm(f => ({ ...f, id_card_type: f.id_card_type === opt.value ? '' : opt.value }))}
                  />
                ))}
              </View>
              ...
              <Row>
                <PhotoUploadField label="ID Card Photo" value={form.id_card_photo_url} onChange={p('id_card_photo_url')} storagePath={`riders/${riderId}/id-card`} aspectRatio={[4, 3]} hint="Front side of ID" />
                <PhotoUploadField label="License Photo" value={form.license_photo_url} onChange={p('license_photo_url')} storagePath={`riders/${riderId}/license`} aspectRatio={[4, 3]} hint="Driving license" />
              </Row>
            </View>
          ...
          {/* Emergency Contact */}
          <Card icon={<Phone size={15} color="#E65100" strokeWidth={2} />} title="Emergency Contact" iconBg="#FFF3E0">
            <Row>
          ...
          {/* Compensation */}
          <Card icon={<IndianRupee size={15} color={Colors.accentDark} strokeWidth={2} />} title="Compensation & Schedule" iconBg={Colors.accentSurface}>
            <Row>
        ...
        <View style={s.footerInner}>
          ...
          <TouchableOpacity style={[s.saveBtn, saving && { opacity: 0.65 }]} onPress={save} disabled={saving} activeOpacity={0.85}>
            {saving
              ...
              : <><Pencil size={16} color={Colors.white} strokeWidth={2} /><Text style={s.saveText}>Update Rider</Text></>
            }
          </TouchableOpacity>
        </View>
      </View>
...
function Card({ icon, title, iconBg, children }: { icon: React.ReactNode; title: string; iconBg: string; children: React.ReactNode }) {
  return (
    <View style={s.card}>
...
const s = StyleSheet.create({
  ...
  loadingText: { fontFamily: Typography.fontFamily.sansRegular, fontSize: Typography.size.sm, color: Colors.textSecondary, marginTop: Spacing[3] },
  fetchErrorText: { fontFamily: Typography.fontFamily.sansSemiBold, fontSize: Typography.size.base, color: Colors.error, textAlign: 'center', marginTop: Spacing[3] },
  backLinkBtn: { marginTop: Spacing[4], paddingVertical: Spacing[3], paddingHorizontal: Spacing[6], borderRadius: Radius.full, borderWidth: 1.5, borderColor: Colors.border },
  backLinkText: { fontFamily: Typography.fontFamily.sansSemiBold, fontSize: Typography.size.base, color: Colors.textSecondary },
  ...
  heroIconCircle: { width: 48, height: 48, borderRadius: 24, backgroundColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  ...
  chip: { borderWidth: 1.5, borderColor: Colors.border, borderRadius: Radius.full, paddingVertical: 7, paddingHorizontal: Spacing[4], backgroundColor: Colors.white },
